...
PE.log = {
  ...
  copyToClipboard() {
    ...
    navigator.clipboard.writeText(el.textContent).then(() => {
      ...
      PE.log.warn('Clipboard access denied');
    });
  },
...
PE.overlay = {
  ...
  clear() {
    const canvas = PE.dom.overlayCanvas;
   ...
   *
   * The overlay is rendered once at full alpha, then the canvas element's
   * CSS opacity is animated via rAF to create the pulse effect. This avoids
   * rewriting pixel buffers every frame.
   *
  ...
  drawSelection(mask, borderDist, borderRadius) {
    ...
    const BASE_ALPHA = 110; // max alpha for interior

    // Render overlay once at full alpha
    ...
    for (let i = 0; i < mask.length; i++) {
      ...
      if (mask[i] === 1) {
        ...
        const fade = 1.0 - ((borderDist[i] - 1) / borderRadius);
        fd[p + 3] = Math.round(BASE_ALPHA * fade);
      }
    }

    ctx.putImageData(frameData, 0, 0);

    // Animate pulse via CSS opacity (0.5 – 1.0, 1.2s sine wave)
    this._startTime = performance.now();
    const self = this;

    function animatePulse(now) {
      const t = (now - self._startTime) / 1200;
      canvas.style.opacity = 0.75 + 0.25 * Math.sin(t * Math.PI * 2);
      self._timer = requestAnimationFrame(animatePulse);
    }
